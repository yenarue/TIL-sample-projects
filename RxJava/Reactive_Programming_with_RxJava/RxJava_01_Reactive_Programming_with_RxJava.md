...
## 결론(?)
RxJava의 타입과 연산자는 **명령형 콜백을 함수형으로 추상화** 시킨 것이다. 이를 자유자재로 사용하려면 함수형 사고로의 전환이 필요해 초반 진입 장벽이 높지만 객체지향과 명령형 스타일에 함수형 스타일을 더하면 엄청난 **시너지 효과**가 일어날 것이다!!! 기왕 쓰는 거 제대로 이해하고 사용해보도록 하자 :-D